...
non_comparative = []
with open('Predicate/Non_comparative.txt','r',encoding='utf-8') as file:
    r = file.readline()
    while r != '':
...
only_files_btc = [f for f in os.listdir("VLSP2023_ComOM_public_test_golden") if
              os.path.isfile(os.path.join("VLSP2023_ComOM_public_test_golden", f))]
for i in only_files_btc:
  df_btc_data=df_btc_data._append(data_utils.preprocessing_with_BIO('VLSP2023_ComOM_public_test_golden/'+i))
